...
Modular, production-grade Infrastructure as Code (IaC) powered by **Terraform / OpenTofu** and the **Cloudflare Provider v5 (`~> 5.0`)** to manage DNS records, perimeter security (Custom WAF Rules, Rate Limiting, Security Headers), edge caching, dynamic portfolio redirects, Email Routing, serverless KV storage, and global zone settings for **[`yeipi.dev`](https://yeipi.dev)**.
...
flowchart TD
    ...
    subgraph CloudflareEdge ["🛡️ Cloudflare Edge Network (yeipi.dev)"]
        ...
        RedirectPhase["🔀 Dynamic Redirects Ruleset\n(/github, /linkedin, /cv, /curriculum, /contact)"]
        WAF["🧱 WAF Custom Rulesets\n(5 Block & Managed Challenge Rules)"]
        RateLimit["⏱️ Rate Limiting\n(5 req / 10s -> Block 10s)"]
        Cache["⚡ Cache Rules\n(Images 2h TTL / Ignore Query String)"]
        Headers["🔒 Security Headers Transform\n(HSTS, X-Frame-Options, CSP, nosniff)"]
        DNS["📡 DNS Layer (Proxied Anycast)\n(A, AAAA, api.yeipi.dev, CNAME, TXT, MX)"]
        EmailRouter["📬 Cloudflare Email Routing & Catch-All"]
    end

    subgraph Origins ["🚀 Origins & Endpoints"]
        GHPages["🐙 GitHub Pages\n(IPv4 / IPv6 Anycast)"]
        WorkersRepo["⚡ cloudflare-workers\n(contact-api, ai-proxy, email-catch-all-reject)"]
        Gmail["✉️ Gmail Destination\n(juanpablo.fernandezdelatorre04@gmail.com)"]
    end

    subgraph StateStorage ["📦 Remote State & Serverless Storage ($0 Cost)"]
        R2["🪣 Cloudflare R2 Bucket\n(terraform-state / S3 Protocol)"]
        KV["🗄️ Cloudflare KV Namespace\n(AI_CACHE)"]
    end

    User --> RedirectPhase
    Crawler --> RedirectPhase
    Attacker --> RedirectPhase

    RedirectPhase -- Matches Shortlink --> ShortlinkRedirect["⚡ 301/302 Redirect"]
    RedirectPhase -- Normal Traffic --> ZoneConfig
    ...
    DNS -- Apex Traffic --> GHPages
    DNS -- api.yeipi.dev --> WorkersRepo
    EmailRouter -- Configured Aliases --> Gmail
    EmailRouter -- Catch-all --> WorkersRepo
    WorkersRepo <--> KV
    GHPages --> Headers --> User

    TerraformCLI["🛠️ Terraform / GitHub Actions"] -->|State Read & Write| R2
    TerraformCLI -->|Provision Resources| CloudflareEdge
    TerraformCLI -->|Provision KV| KV
```

### 🔗 Relationship with `cloudflare-workers`

This repository (`cloudflare-infra`) provisions and manages the foundational **DNS, Edge Security, and Infrastructure Storage**. It coordinates directly with the companion repository **[`cloudflare-workers`](https://github.com/yeipis/cloudflare-workers)**:
* **Infrastructure Layer (`cloudflare-infra`):** Declares DNS records (`api.yeipi.dev`), WAF security expressions, rate limiters, A+ Security Headers, shortlink redirects, KV namespaces (`AI_CACHE`), and Email Routing Catch-all.
* **Application / Serverless Layer (`cloudflare-workers`):** Implements backend APIs (`contact-api`, `ai-proxy`) and the serverless email handler (`email-catch-all-reject`), consuming the `ai_cache_kv_id` generated by this repository.
...
├── modules/
│   ├── dns/                          # DNS records (A, AAAA, api.yeipi.dev, CNAME, MX, TXT)
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   ├── outputs.tf
│   │   └── versions.tf
│   ├── rulesets/                     # WAF, Rate Limiting, Cache, Security Headers & Redirects
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   ├── outputs.tf
│   │   └── versions.tf
│   ├── email/                        # Email Routing forwarding & catch-all rules
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   ├── outputs.tf
│   │   └── versions.tf
│   ├── workers-kv/                   # Cloudflare Workers KV Namespaces (AI_CACHE)
│   │   ├── main.tf
│   │   ├── variables.tf
│   │   ├── outputs.tf
│   │   └── versions.tf
...
├── outputs.tf                        # Summary outputs and resource IDs (includes ai_cache_kv_id)
...
| :--- | :--- | :--- |
| `CLOUDFLARE_API_TOKEN` | Cloudflare API Token with Zone, DNS, WAF, Email, KV, Transform permissions | Cloudflare Dashboard > API Tokens |
...
* **AAAA Records (Proxied):**
  * Apex `yeipi.dev` -> GitHub Pages Anycast IPv6 (`2606:50c0:8000::153`, `2606:50c0:8001::153`, `2606:50c0:8002::153`, `2606:50c0:8003::153`).
  * `api.yeipi.dev` -> `100::` (Proxied API Gateway routing traffic to Cloudflare Workers).
...
* **WAF Custom Rules (`http_request_firewall_custom`):**
  *(All 5 rules are scoped to protect `yeipi.dev` and `www.yeipi.dev` while exempting `api.yeipi.dev` to allow programmatic microservice traffic within Cloudflare Free plan's 5-rule limit).*
...
* **Dynamic Redirects (`http_request_dynamic_redirect`):**
  ...
  * `/github`, `/gh` ➡️ `https://github.com/yeipis` (301)
  * `/linkedin`, `/in` ➡️ `https://www.linkedin.com/in/jpfdlt/` (301)
  * `/cv`, `/curriculum` ➡️ CV / Portfolio destination (302)
  * `/contact`, `/mail`, `/email` ➡️ `mailto:juanpablo@yeipi.dev` (302)

### 3. Serverless Storage (`modules/workers-kv/`)
* **KV Namespaces:**
  * `AI_CACHE`: Edge KV store used by `ai-proxy` to cache LLM prompt completions.
  * Outputs `ai_cache_kv_id` to link directly into `wrangler.jsonc`.

### 4. Email Routing (`modules/email/`)
* **Forwarding Rules:** Forwards inbound emails from aliases to `juanpablo.fernandezdelatorre04@gmail.com`:
  * `admin@yeipi.dev`, `contacto@yeipi.dev`, `contact@yeipi.dev`, `jp@yeipi.dev`, `juanpablo@yeipi.dev`.
* **Catch-All Rule:** Routes all unmapped inbound emails to the `email-catch-all-reject` worker.

### 5. Zone Settings (`modules/zone-settings/`)
* `ssl = "strict"`
* `always_use_https = "on"`
* `min_tls_version = "1.2"`
* `http3 = "on"`
...
* [`cloudflare_ruleset` Documentation](https://registry.terraform.io/providers/cloudflare/cloudflare/latest/docs/resources/ruleset) — Resource specification for WAF, Rate Limiting, Cache, Transform, and Redirect rulesets.
* [`cloudflare_workers_kv_namespace` Documentation](https://registry.terraform.io/providers/cloudflare/cloudflare/latest/docs/resources/workers_kv_namespace) — Resource specification for KV namespaces.
* [`cloudflare_email_routing_catch_all` Documentation](https://registry.terraform.io/providers/cloudflare/cloudflare/latest/docs/resources/email_routing_catch_all) — Resource specification for Catch-All email routing.
...
* [Cloudflare Transform Rules (HTTP Headers)](https://developers.cloudflare.com/rules/transform/) — Modifying request and response HTTP headers at the edge.
* [Cloudflare Dynamic Redirects](https://developers.cloudflare.com/rules/url-forwarding/) — URL forwardings and shortlinks at the edge.
* [Cloudflare Workers KV](https://developers.cloudflare.com/kv/) — Global low-latency key-value data storage.
